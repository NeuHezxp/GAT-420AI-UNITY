...
public class AIUtilityWaypoints : MonoBehaviour
{
	...
	IEnumerator MoveToRandomWaypoint(float timer)
	{
		...
		agent.movement.MoveTowards(waypoints[Random.Range(0, waypoints.Length)].transform.position);
		// wait until distance between agent position to agent movement destination is < 1
		yield return new WaitUntil(() => Vector3.Distance(agent.transform.position, agent.movement.Destination) < 1); //()=> is a predicate that returns a boolean
